...
class Like
	@debug = true
	likedDate = Date.now() / 1000
	currentOffset = 0
	processing = false

	@get = (runs) ->
		return if processing
		console.log "getting likes before  " + new Date(likedDate * 1000) if @debug
		processing = true
		...
		# --- GET /v2/user/likes ---
		Tumblr.get "https://api.tumblr.com/v2/user/likes?before="+likedDate
			.done (data) ->
				...
				likedDate = data.response.liked_posts[posts.length - 1].liked_timestamp
